          ...
          {FILTERS.map((filter) => {
            ...
            return (
              <Pressable
                ...
                <Text className={`text-center text-sm font-semibold ${isActive ? 'text-white' : 'text-slate-600'}`}>
                  {filter.label}
                </Text>
              </Pressable>
            );
      ...
      <FlatList
        ...
        ListEmptyComponent={
          loading ? (
            ...
            <View className="mt-20 items-center">
              <Text className="text-sm text-slate-400">No bookings in this category yet.</Text>
            </View>
          )
        }
      />
          ...
          <View className="rounded-t-[32px] bg-white px-6 pb-8 pt-6">
            {selectedBooking ? (
              <>
                ...
                <LoadingImage
                  source={{ uri: selectedBooking.coverImage ?? LISTINGS[0]?.coverPhoto ?? AVATARS[0] }}
                  style={{ height: 192, width: '100%' }}
                  className="rounded-3xl"
                />
                <View className="mt-4 flex-row items-center justify-between">
                  <View>
                    <Text className="text-xs uppercase tracking-[0.3em] text-blue-500">Guest</Text>
                    <Text className="text-base font-semibold text-slate-900">
                      {selectedBooking.guestName}
                    </Text>
                  </View>
                  <View className="items-end">
                    <Text className="text-xs uppercase tracking-[0.3em] text-blue-500">Stay</Text>
                    <Text className="text-base font-semibold text-slate-900">
                ...
                <Pressable
                  ...
                  <Text className="ml-2 text-base font-semibold text-white">View Apartment</Text>
                </Pressable>
              </>
            ) : null}
          </View>
